...
rm -rf /var/www/html/* && cd /var/www/html && tar -zxvf /migration/backup_osclass.tar.gz
...
mysql -uroot -p${MYSQL_ROOT_PASSWORD} -h mysql -e "CREATE DATABASE $OSCLASS_DB_NAME DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;"
mysql -uroot -p${MYSQL_ROOT_PASSWORD} -h mysql -e "CREATE USER $OSCLASS_USER_NAME@'%' IDENTIFIED BY '$OSCLASS_PASSWD'"
mysql -uroot -p${MYSQL_ROOT_PASSWORD} -h mysql -e "GRANT ALL PRIVILEGES ON $OSCLASS_DB_NAME.* TO $OSCLASS_USER_NAME@'%' WITH GRANT OPTION"
